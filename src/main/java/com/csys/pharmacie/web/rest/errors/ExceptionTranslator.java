package com.csys.pharmacie.web.rest.errors;

import com.csys.exception.IllegalBusinessLogiqueException;
import com.csys.exception.MyResourceNotFoundException;
...
public class ExceptionTranslator {
    ...
    @ResponseBody
    @ResponseStatus(HttpStatus.CONFLICT)
    public ResponseEntity<ErrorVM> processMethodOptimisticLockingException(StaleStateException exception) {
        Locale loc = LocaleContextHolder.getLocale();
        Object[] obj = new Object[1];
        if (exception.getCause() != null) {
            obj = exception.getCause().getMessage().split(";");
        ...
        String translatedmessage = messages.getMessage("optimitic-lock-message-exception", obj, loc);
        BodyBuilder builder;
        builder = ResponseEntity.status(HttpStatus.CONFLICT);
        return builder.body(new ErrorVM("CONFLICT", translatedmessage));
    }
